...
export async function create(body: CreateStandardBody) {
  ...
  return prisma.maintenanceStandard.create({
    data: {
      ...rest,
      ...(scheduleItems ? { scheduleItems: { create: scheduleItems } } : {})
    },
  ...
  try {
    ...
    if (
      ...
      throw new AppError(
        409,
        'STANDARD_IN_USE',
        'Standard has schedule items still tracked on a vehicle'
      );
    }
    throw err;
  }
...
export async function addScheduleItem(
  standardId: string,
  body: CreateScheduleItemBody
) {
  await getById(standardId);
  return prisma.maintenanceScheduleItem.create({
    data: { ...body, maintenanceStandardId: standardId }
  });
}

export async function removeScheduleItem(itemId: string): Promise<void> {
  const item = await prisma.maintenanceScheduleItem.findUnique({
    where: { id: itemId }
  });
  ...
  try {
    ...
    if (
      ...
      throw new AppError(
        ...
        'Schedule item is tracked on a vehicle'
      );
    }
    throw err;
  }
