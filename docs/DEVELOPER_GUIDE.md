# Motorpool Management System — Developer Guide

> For a non-technical overview of **how the system works** (roles, workflows, screens), see the [README](../README.md). This guide covers the technical side: setup, API reference, and configuration.
...
| `POST /api/trip-tickets/:id/cancel`               | Owning requester or admin, from either pending state or `approved`; body requires `reason`. Mirrors `cancelled` onto the fuel allocation if one exists.                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `POST /api/trip-tickets/:id/dates/:dateId/cancel` | Owning requester or admin. Cancels ONE date on the ticket without voiding the rest of the event — legal only while the ticket is `approved`/`in_progress` and the date is still `scheduled`; body requires `reason`. If cancelling this date leaves every date settled with none completed, the ticket itself derives to `cancelled` — at that point (and only then) this settles exactly like whole-ticket cancel: the ticket's own `cancellationReason` is recorded, its fuel allocation is mirrored to `cancelled`, and the `trip_cancelled` notification fires instead of naming just the one date. |
